...
class Graph {
 public:
  ...
  bool check_symmetry();
  /*read a graph from a txt file (with format src_node dst_node) and generate binary graph*/
  void txt_file2binary_graph(char* filename, char* filename_w, int type);
  void adjacency_graph2binary_graph(vector<vector<int>> adj_g, char* filename_w, int type);
