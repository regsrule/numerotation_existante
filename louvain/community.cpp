...
Community::Community(vector<vector<int>> adj_g, int type, int nbp, double minm) {

//  g = Graph();
  g.adjacency_graph2binary_graph(adj_g, NULL, type);
  size = g.nb_nodes;
...
Community::Community(char * txt_filename, int type, int nbp, double minm) {

//  g = Graph();
  g.txt_file2binary_graph(txt_filename, NULL, type);
  size = g.nb_nodes;

  adjacency_graph.resize(size);
  neigh_weight.resize(size,-1);
  neigh_pos.resize(size);
  neigh_last=0;
  ...
  for (int i=0 ; i<size ; i++) {
    ...
    tot[i] = g.weighted_degree(i);
    in[i]  = g.nb_selfloops(i);
	adjacency_graph[i].resize(g.nb_neighbors(i));//to capture the adjacency graph
	pair<vector<unsigned int>::iterator, vector<float>::iterator > p = g.neighbors(i);
	for(unsigned int k = 0; k < g.nb_neighbors(i); k++) {
		adjacency_graph[i][k] = *(p.first+k);
	}
  }
...
Community::store_partition_louvain(bool store_affinity) {
  vector<int> renumber(size, -1);
  for (int node=0 ; node<size ; node++) {
    renumber[n2c[node]]++;
  }
  ...
  vector<vector<int> > scomm(final_nb);
  comm_affinity.resize(final_nb);
  for (int i=0 ; i<size ; i++){
  //  cout << i << " " << renumber[n2c[i]] << endl;
    scomm[renumber[n2c[i]]].push_back(i);
    if(store_affinity)
    {
	pair<vector<unsigned int>::iterator, vector<float>::iterator > p = g.neighbors(i);
	//cout << i <<"("<<g.nb_neighbors(i) << "):" ;
	if(comm_affinity[renumber[n2c[i]]].size() == 0)
		comm_affinity[renumber[n2c[i]]].resize(final_nb,0);
	for(unsigned int k = 0; k < g.nb_neighbors(i); k++) {
		comm_affinity[renumber[n2c[i]]][renumber[n2c[*(p.first+k)]]] += (g.weights.size()==0)?1.:*(p.second+k);
	}
...
bool
Community::one_level(int min_improvement) {
...
//      int node = node_tmp;
    ...
    new_mod = modularity();
    if (nb_moves>min_improvement)
      improvement=true;
    
  } while (nb_moves>0 && (new_mod-cur_mod)>min_modularity && nb_pass_done<nb_pass);
