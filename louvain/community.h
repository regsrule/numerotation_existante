...
class Community {
 public:
  ...
  // read  graph from txt file instead of binary file
  Community (char *txt_filename, int type, int nb_pass, double min_modularity);
  // read  graph from an existing adjacency graph instead of binary file
  Community(vector<vector<int>> adj_g, int type, int nbp, double minm);
  // copy graph
  ...
  // store the graph of communities as computed by one_level
  std::vector<std::vector<int> > store_partition_louvain(bool store_affinity);
  ...
  bool one_level(int min_improvement);
};
